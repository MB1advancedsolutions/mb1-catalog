...
(function(){
  ...
  let originalOpenProductData = null;
  ...
  function installStyleUI(){
    ...
    style.textContent=`
      .mb1-style-box{margin-top:14px!important;border:1px solid #d9dfda!important;border-radius:14px!important;background:#f8faf8!important;padding:14px!important;max-width:100%!important;box-sizing:border-box!important}
      .mb1-style-box label{display:block!important;font-size:15px!important;font-weight:800!important;color:#1f2822!important;margin:0 0 8px!important}
      #mb1StyleSelect{width:100%!important;max-width:100%!important;padding:9px 10px!important;border:1px solid #cfd7d0!important;border-radius:9px!important;background:#fff!important;margin-bottom:10px!important}
      .mb1-style-thumbs{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:8px!important;max-height:330px!important;overflow-y:auto!important;overflow-x:hidden!important;padding:2px!important;scrollbar-width:thin!important}
      .mb1-style-thumb{position:relative!important;display:flex!important;flex-direction:column!important;min-width:0!important;width:auto!important;border:1px solid #dce2dd!important;border-radius:10px!important;background:#fff!important;padding:6px!important;cursor:pointer!important;text-align:left!important;color:var(--ink)!important}
      .mb1-style-thumb:hover{border-color:#91a095!important;box-shadow:0 4px 12px rgba(32,50,38,.09)!important}
      .mb1-style-thumb.active{border:2px solid #2f6845!important;padding:5px!important;box-shadow:0 0 0 2px rgba(47,104,69,.10)!important}
      .mb1-style-thumb.active:after{content:'✓';position:absolute;right:6px;top:6px;width:20px;height:20px;border-radius:50%;display:grid;place-items:center;background:#2f6845;color:#fff;font-size:12px;font-weight:900}
      .mb1-style-thumb img{width:100%!important;aspect-ratio:1/1!important;height:auto!important;display:block!important;object-fit:contain!important;background:#f7f6f2!important;border-radius:7px!important;margin:0!important}
      .mb1-style-thumb span{display:block!important;font-size:11px!important;font-weight:800!important;line-height:1.15!important;margin:6px 1px 1px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
      .mb1-style-selected{font-size:12px!important;color:#44564a!important;margin-top:9px!important;font-weight:800!important;padding:8px 9px!important;background:#edf4ef!important;border-radius:8px!important}
      .card .style-count{font-size:11px;color:var(--muted);margin-top:4px}
      @media(max-width:760px){.mb1-style-thumbs{grid-template-columns:repeat(3,minmax(0,1fr))!important;max-height:300px!important}}
      @media(max-width:480px){.mb1-style-thumbs{grid-template-columns:repeat(2,minmax(0,1fr))!important;max-height:340px!important}}
    `;
  ...
  function finishBoot(){
    ...
    console.info('MB1 grouped storefront V23 loaded',window.MB1_GROUPING_STATUS);
    ...
    const refresh=setInterval(()=>{
      ...
      if((PRODUCTS||[]).length){
        ...
        buildCategoryButtons();
        render();
        const grouped=buildDisplayProducts().filter(p=>p._mb1Grouped).length;
        window.MB1_GROUPING_STATUS.groupedCards=grouped;
        window.MB1_GROUPING_STATUS.productCount=(PRODUCTS||[]).length;
        if(grouped>0||tries>=24)clearInterval(refresh);
      }else if(tries>=24)clearInterval(refresh);
    },250);
  }
  ...
  (function waitForCatalog(){
    ...
    const wait=setInterval(()=>{
      ...
      const ready=
        ...
        typeof buildCategoryButtons==='function' &&
        typeof openProductData==='function' &&
        typeof isCustomizableProduct==='function' &&
      ...
      if(ready){
        ...
        clearInterval(wait);
        console.error('MB1 V23 grouping could not initialize because the catalog environment was not ready.');
      }
    },100);
  })();
})();
