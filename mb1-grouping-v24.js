...
(function(){
  ...
  function applyOverrides(){
    ...
    render=function(){
      ...
      grid.innerHTML=filtered.map(p=>`<article class="card"><div class="art"><img loading="lazy" src="${esc(p.thumb)}" alt="${esc(p.alt||p.name)}"></div><div class="info"><div class="cat">${esc(categoryLabel(c||p.category))}</div><div class="name">${esc(p.name)}</div>${p._mb1Grouped?`<div class="style-count">${p.children.length} styles in one item</div>`:''}<div class="meta">${esc(p._mb1Grouped?(p.children.length+' design options'):p.sku)}<br><span class="price">From $65</span> - 8&quot;-48&quot;</div><button class="view-button" data-display-key="${esc(p._mb1Grouped?p.groupId:norm(p.design))}">${p._mb1Grouped?'Choose Style':'View Design'}</button></div></article>`).join('');
