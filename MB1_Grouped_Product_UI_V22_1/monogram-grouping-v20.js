/* MB1 Product Grouping V22.1 - clean grouped-product configurator with embedded V20+V21 fallback
...
Promise.all([
  fetch('monogram-groups-v20.json?v=221')
    .then(r=>{if(!r.ok)throw new Error('Monogram grouping config '+r.status);return r.json()})
    .catch(err=>{console.warn('Using embedded V20 grouping backup:',err);return EMBEDDED_BASE_CONFIG}),
  fetch('catalog-groups-v21.json?v=221')
    .then(r=>{if(!r.ok)throw new Error('Catalog grouping config '+r.status);return r.json()})
    .catch(err=>{console.warn('Using embedded V21 grouping backup:',err);return EMBEDDED_V21_CONFIG})
]).then(([baseCfg,extraCfg])=>{
  // Merge embedded backups first, then live files. Missing groups in a stale live file remain preserved.
  CONFIG=mergeGroupConfigs(EMBEDDED_BASE_CONFIG,EMBEDDED_V21_CONFIG,baseCfg,extraCfg);
  buildMaps();
  applyOverrides();
}).catch(err=>{
  // Last-resort path: the UI still loads from the embedded combined config.
  console.warn('Loading embedded combined MB1 grouping config:',err);
  CONFIG=mergeGroupConfigs(EMBEDDED_BASE_CONFIG,EMBEDDED_V21_CONFIG);
  buildMaps();
  applyOverrides();
});
